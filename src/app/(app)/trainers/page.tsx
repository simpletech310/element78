      ...
      <div className="app-scroll app-top" style={{ paddingBottom: 100 }}>
        ...
        {featured && (
          <div style={{ padding: "14px 22px 4px" }}>
            ...
            <Link href={`/trainers/${featured.slug}`} style={{ position: "relative", borderRadius: 18, overflow: "hidden", height: 320, display: "block", color: "var(--bone)" }}>
              <Photo src={featured.hero_image ?? ""} alt={featured.name} style={{ position: "absolute", inset: 0 }} />
        ...
        <div style={{ padding: "20px 22px 0", display: "flex", flexDirection: "column", gap: 10 }}>
          {rest.map(t => (
            <Link key={t.id} href={`/trainers/${t.slug}`} style={{ display: "flex", gap: 14, padding: 12, borderRadius: 14, background: "var(--paper)", border: "1px solid rgba(10,14,20,0.06)", color: "var(--ink)" }}>
